(() => {
  ...
  function renderSR() {
    ...
    // falloff like Genshin). One even linear fade: 0 at the zero point
    // (~4 lines above the name), ramping up to full strength at the very
    // bottom of the image — no plateau.
    ...
    if (bgRowTop < H) {
      ...
      for (let row = 0; row < region.height; row++) {
        ...
        const t = bgFadeReach > 0 ? Math.min(Math.max((y - bgStartY) / bgFadeReach, 0), 1) : 1;
  ...
  function render() {
    ...
    ctx.font = `${Math.round(32 * s)}px ${FONT}`;
